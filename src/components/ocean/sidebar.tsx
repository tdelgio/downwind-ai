...
import { CalendarDays, Fish, Home, Waves } from "lucide-react";
...
export function Sidebar({ active }: { active: string }) {
  return (
    <aside className="hidden w-64 shrink-0 border-r border-[rgba(9,76,96,0.08)] bg-white/[0.62] p-4 backdrop-blur-[22px] lg:block">
      <Link href="/home" className="flex items-center gap-3 rounded-2xl bg-[#f7fafa] p-3 ring-1 ring-[#e4e8ea]">
        <span className="grid size-11 place-items-center rounded-full bg-white text-[#0d5968] ring-1 ring-[#d8e4e7]">
          <Waves className="size-5" />
        </span>
        <span>
          <span className="block text-base font-semibold tracking-[0.01em] text-[#102b3a]">Downwind AI</span>
          <span className="block text-xs font-medium uppercase tracking-[0.12em] text-[#6e8188]">Maui forecast</span>
        </span>
      </Link>
        ...
        {navItems.map((item) => (
          <Link
            ...
            className={cn(
              ...
              active === item.href ? "bg-[#102b3a] text-white" : "text-[#5f7078] hover:bg-[#f3f7f8] hover:text-[#102b3a]",
            )}
          >
